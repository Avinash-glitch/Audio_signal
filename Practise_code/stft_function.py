...
def main(inputFile = "D:\Projects\sms-tools-master\sms-tools-master\sounds\speech-female.wav", window = 'hamming', M = 1024, N = 1024, H = 512):
	"""
	...
	frmTime = H*np.arange(numFrames)/float(fs)
	binFreq = fs*np.arange(N*maxplotfreq/fs)/N
	plt.pcolormesh(frmTime, binFreq, np.diff(pX[:,:int(N*maxplotfreq/fs)],axis=1))
	plt.xlabel('time (sec)')
	plt.ylabel('frequency (Hz)')
