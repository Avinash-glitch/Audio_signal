...
"""
def estimateInharmonicity(inputFile = "D:\Projects\sms-tools-master\sms-tools-master\sounds\piano.wav", t1=0.2, t2=0.4, window='hamming', 
    ...
    #N1 = int(t1 * float(fs) / H)
    #N2 = int(t2 * float(fs) / H)
    total_time = float(len(x)) / fs
    
    N1 = int(np.ceil((t1 / total_time) * len(xfreq)))
    N2 = int(np.ceil((t2 / total_time) * len(xfreq)))
    ...
    for i in range(N1,N2+1):
        sum_array = np. array([])
        for r in range(1,nH):
            sum_array= np.append(sum_array,abs(xfreq[i,r-1] - (r) * xfreq[i,0])/r)
        I= np. append(I,np.sum(sum_array)/len(sum_array))

    I_mean =  np.sum(I)/len(I)
    return I_mean
